...
- (instancetype)init {
    self = [super init];
    _annotationSize = CGSizeMake(20, 20);
    _image = [UIImage imageNamed:@"marker"];
    _annotationView = [[BMKAnnotationView alloc] initWithAnnotation:self reuseIdentifier:@"RTCMARKERCustom"];
...
- (void)setSize:(CGSize)size{
    _annotationView.bounds = CGRectMake(0, 0, size.width, size.height);
...
- (void)didAddSubview:(UIView *)subview {
    if ([subview isKindOfClass:[RCTCallout class]]) {
        ...
        _annotationView.bounds = CGRectMake(0, 0, _annotationSize.width, _annotationSize.height);
